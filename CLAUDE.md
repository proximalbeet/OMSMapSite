...
A WordPress plugin that renders an interactive world map of the OMS (Opsoclonus Myoclonus Syndrome) community. It supports two data layers — patients by country and OMS hospitals/hospitals — with a 3-way toggle to switch between views. Data is pulled live from two public Google Sheets and cached server-side.
...
1. **Google Sheets → PHP** (`MapPlugin.php`): On each page load, two functions fetch CSV exports from their respective configured Google Sheets via `wp_remote_get`. Both are cached in WordPress transients for 1 hour and invalidated when settings are saved.
   ...
   - `map_plugin_get_hospitals_data()` — hospitals sheet (`institution, hospital, address, phone, lat, lng`)
...
2. **PHP → JavaScript** (`MapPlugin.php:44–47`): Both datasets injected into the page as `MapPluginData.features` and `MapPluginData.hospitals` via `wp_localize_script`.
...
Orange bubble markers sized logarithmically: `Math.max(28, Math.min(64, Math.log(count + 1) * 10))`. Each bubble shows the patient count. `zIndexOffset: -1000` keeps them below hospital markers. Clicking opens a popup with country name and count badge. Unknown-location patients are placed at `lat: -40, lng: -160` (South Pacific).

### Hospital Layer

Green circle markers, sized by `OMAS Cases` (patients served): <20 → 28px, 20–50 → 36px, 50–100 → 46px, >100 → 58px. `zIndexOffset: 1000` ensures they always render on top of patient bubbles. Clicking opens a popup with institution name (linked to their website if a `url` is present), hospital name, address, and a `tel:` phone link.

### Header Stats

Total patient count (sum of all `count` values) and total hospital count are computed in JS and injected as stat figures into the header. Only shown when data is present.

### Toggle

Three-button segmented control (Patients | Both | Hospitals) injected into the header by JS. Hidden entirely if no hospitals sheet is configured — zero visible change for unconfigured installs. Default state is Patients.
...
- **Hospitals Google Sheet URL** → stored as `map_plugin_hospitals_sheet_url`
...
└── assets/
    ...
    └── spreadsheets/
        ...
        └── hospitals_map.csv # Clean hospital data ready for Google Sheets import (4 hospitals, fixed coordinates)
```
...
| OpenStreetMap tiles | Map tile layer | `tile.openstreetmap.org` (fetched client-side; previously CARTO Voyager tiles, switched off after CARTO began requiring an API key and stamping an "API KEY REQUIRED" watermark on unauthenticated requests — old CARTO layer left commented out in `MapPlugin.js` for reference) |
| Google Sheets CSV export | Patient + hospital data | Server-side `wp_remote_get` |
...
**Hospitals sheet** — row 1 headers: `Institution, Hospital, Address, Phone number, Type, OMAS Cases, Registry, Picture, Video, lat, lng`
...
- `Picture` — URL to a photo of the hospital (shown at top of popup if present)
...
- Use `hospitals_map.csv` as the import template — it has clean coordinates and all column headers
...
- `.map-stat-number` — large bold figures for patient/hospital totals
- `.map-bubble` — orange patient markers, logarithmic size, `zIndexOffset: -1000`
- `.map-hospital-marker` — green `+` markers, sized by `OMAS Cases` via `--size-1`..`--size-4` modifier classes (28–58px), `zIndexOffset: 1000`
...
- `.map-plugin-attribution` — tiny faded text below disclaimer, links to OSM
